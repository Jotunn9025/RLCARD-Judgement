...
class JudgementGame:
    ...
    def _calculate_round_payoffs(self)->List[float]:
        """
        Calculates scores for the round based on standard Oh Hell rules.
        Success: 10 + bid.
        Failure: -10 * error.
        This provides a natural soft penalty while keeping absolute scores readable.
        """
        payoffs = []
        for player in self.players:
            error = abs(player.bid - player.tricks_won)
            if error == 0:
                score = 10.0 + player.bid
            else:
                score = -10.0 * error
            payoffs.append(float(score))
        return payoffs
