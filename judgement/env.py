...
class JudgementEnv(Env):
    ...
    def _extract_state(self, state:Dict)->Dict:
        ...
        #bdis
        max_cards = state.get('num_cards', 13)
    ...
    def get_payoffs(self) -> np.ndarray:
        """
        Get zero-sum relative payoffs normalized for RLCard agents.
        Calculates each player's score relative to the average score of opponents.
        """
        raw_scores = np.array(self.game.get_payoffs(), dtype=np.float32)
        relative_scores = np.zeros(self.NUM_PLAYERS, dtype=np.float32)
        
        for i in range(self.NUM_PLAYERS):
            opponents_scores = np.delete(raw_scores, i)
            # Relative score: How much better am I doing than the average opponent?
            relative_scores[i] = raw_scores[i] - np.mean(opponents_scores)
            
        # Scale to manageable neural network ranges (keeping exact zero-sum property)
        return relative_scores / 20.0
